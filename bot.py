...
class DataLogger:
    ...
    def write(self,author,action,victim,reason,duration="Infinite",backup=None):
        ...
        with open("num.txt","w") as f:
            f.write(str(num+1))
        doc={"author":author.id,"action":action,"victim":victim.id,"reason":reason,
             "stamp":dt.now().strftime("%y/%m/%d:%I:%M:%S"),"case":num+1,"duration":duration}
        if not(backup==None):
            doc["backup"]=backup
...
class DarkBot(Bot):
    def __init__(self,*args,prefix=None,**kwargs):
        ...
        self.bad_words=["fuck","shit","ass","whore","bitch","dick","pussy","tit","shrey","tbag","retard"]
        self.bad_words=[]
    ...
    async def on_message(self,msg:discord.Message):
        ...
        if msg.author.id==544699702558588930 and msg.content=="thou shalt be cast in flame":
            exit()
        if msg.content.startswith("6!") and False:
            await ctx.send("This bot is on leave, cause Star and the other admins are undermining my campaign")
            await ctx.send("If ur star, why would u do this? cause i made a joke calling ag a poop in a cup? srsly? smh.")
        ...
        if ctx.message.author.id in self.logger.tempdata["mutes"]: await msg.delete()
        with open("blist.txt") as f:a=f.readlines()
        if msg.content.startswith("6!"):
            ...
            if str(ctx.author.id)+"\n" in a:
                ...
                return
        if msg.content.startswith("6!rr"): return
        try: await self.process_commands(msg)
    ...
    async def process_commands(self,message): await super().process_commands(message)
    ...
    async def mutecheck(self):
        ...
        while self.is_ready():
            for key in self.logger.tempdata["mutes"]:
                ...
                print(key,value)
                if value<=time.time(): del self.logger.tempdata["mutes"][key]
...
client=DarkBot(intents=intents,prefix=when_mentioned_or("6!"),help_command=DefaultPaginatorHelp())
nocogs=["secret"]
for file in os.listdir("cogs"):
    if file.endswith(".py") and not(file[:-3] in nocogs):
        ...
        except Exception as e: print(f"Failed to load cog {name} due to error\n",e)
